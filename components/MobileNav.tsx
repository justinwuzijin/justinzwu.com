...
const socialLinks = [
  ...
  { 
    name: 'GitHub', 
    url: 'https://github.com/justinwuzijin',
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
  ...
  { 
    name: 'LinkedIn', 
    url: 'https://www.linkedin.com/in/justinzijinwu/',
    icon: (
      <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
...
export function MobileNav() {
  ...
  // Auto-expand social links after delay when menu opens
  useEffect(() => {
    ...
    <>
      ...
      <aside className={`${styles.sidebar} ${isOpen ? styles.sidebarOpen : ''}`}>
        <nav className={styles.nav} aria-label="Main navigation">
          ...
          {/* Social Links - stacked with expand animation */}
          <div className={styles.socialLinksContainer}>
            <div className={styles.socialLinksStack}>
              {socialLinks.map((link, index) => (
                <motion.a
                  ...
                  initial={false}
                  animate={{
                    x: socialExpanded ? index * 44 : 0,
                    opacity: socialExpanded ? 1 : (index === 0 ? 1 : 0),
                  }}
                  transition={{
                    type: 'spring',
                    stiffness: 400,
                    damping: 30,
                    delay: socialExpanded ? index * 0.05 : (socialLinks.length - index - 1) * 0.03,
                  }}
                  style={{
                    zIndex: socialLinks.length - index,
                    pointerEvents: socialExpanded ? 'auto' : (index === 0 ? 'auto' : 'none'),
                  }}
                >
                  {link.icon}
                </motion.a>
